...
def build_coordinator(
    ...
    async def dispatch_specialists(request: str, domains: list[str]) -> str:
        ...
        # TODO(exercise 7): This is the parallel-vs-sequential fork this repo's evals
        # --compare exists to demonstrate.
        #   - When `multiagent` is True: run every chosen specialist CONCURRENTLY with
        #     asyncio.gather(*(_run_specialist(d, request) for d in chosen)) — each one
        #     already forks its own isolated memory branch inside _run_specialist.
        #   - When `multiagent` is False: run them one at a time in a plain loop
        #     (`for d in chosen: results.append(await _run_specialist(d, request))`).
        #     _run_specialist already skips branch forking when multiagent is False,
        #     so this path is a fair "no multi-agent machinery" baseline, not just a
        #     slower version of the same thing.
        # Join `results` into the same "[{domain}]\n{result}" format either way.
        raise NotImplementedError("TODO: gather specialists concurrently (multiagent) or sequentially (single-agent)")
